    ...
    ui(new Ui::admin)
{
    ...
    ui->tabWidget->setCurrentIndex(0);//默认显示tableWidget第0个布局
    ...
    connect(ui->btn_stu_menu_1,&QPushButton::clicked,[=](){model_stu->select();});
...
void admin::on_btn_stu_3_3_clicked()
{
    ...
    if (line_text == "关闭"){//选课系统关闭后，检测课程人数，若小于15人，则删除该课程的所有选课记录
...
void admin::on_btn_dept_query_clicked()
{
    ...
    if (text.length()==4 && text[0]>='0' && text[0]<='9')
        str = QString("select D.dept_id, D.dept_name, T.tch_name "
                              "from department D, teacher T "
                              "where D.dept_master=T.tch_id AND D.dept_id='%1'").arg(text);
    else
        str = QString("select D.dept_id, D.dept_name, T.tch_name "
                              "from department D, teacher T "
                              "where D.dept_master=T.tch_id AND D.dept_name='%1'").arg(text);
    query.exec(str);
    query.next();
    ui->lineEdit_dept_1->setText(query.value(0).toString());
    ui->lineEdit_dept_2->setText(query.value(1).toString());
    ui->lineEdit_dept_3->setText(query.value(2).toString());
}
