...
void home::init_score(){
    ...
    QString course_name,course_type,course_credit,score,again;
    QString  search = QString("select C.course_name,C.course_type,C.course_credit,S.score,S.again "
    ...
    while (query.next()) {
        ...
        QString tmp = QString("%0*%1*%2*%3*%4").arg(course_name).arg(course_type).arg(course_credit).arg(score).arg(again);
...
void  home::init_checkin(){
    QString stu_name,stu_gen,stu_age,stu_deptid,stu_dept,is_check;
    ...
    while(sql_query.next()){
        ...
        stu_deptid = sql_query.value(4).toString();
        is_check = sql_query.value(6).toString();
    }
    if(is_check == 'Y'){
        ui->pbtn_ok->setEnabled(false);
    }
        QString search1 = QString("select dept_name from department where dept_id = '%1'").arg(stu_deptid);
...
void home::on_pbtn_ok_clicked()
{
    ...
    QString stu_dept = ui->le_dept->text();
...
\
    ...
    if (!count);
        ...
        if(total_credit>120){
            ...
            for (int i=0; i<count;i++) {
                stat = QString("insert into selection values('%1','%2',0,'N')").arg(text).arg(ui->tableWidget_1->item(i,0)->text());
                if (!query.exec(stat)) {
